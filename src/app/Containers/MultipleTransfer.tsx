...
const MultipleTransfer = ({ title, state, setState, onTransferCountChange, block, formData }: CombineFormProps) =>  {
  ...
  const numberOfTransfers = [1, 2, 3, 4, 5];
  ...
  const handleVehicleFieldChange = (label: string, value: string | boolean) =>  {
    console.log("handleVehicleFieldChange in Multiple", label, value);
    const current = getCurrentTransfer();
    updateCurrentTransfer({
      vehicleInfoState: {
  ...
  // ===> Handler for legal owner fields
  ...
  useEffect(() =>  {
    ...
    if (savedState) {
      ...
      setMultipleTransfer(parsed.multipleTransfer || [{ ...initialTransferState, transferNumber: 1 }]);
    }
  }, []);
    ...
    <div> 
      <div className="flex items-center gap-4 mb-4"> 
        ...
        <CustomDropdown
          ...
          options={numberOfTransfers.map((num) => ({ label: num.toString(), value: num }))}
        />
        </div>
        {/* <select
          value={transferCount}
      ...
      <div className="space-y-6"> 
        ...
        {LegalOwnerOfRecordBlock && isThereIsACurrentLeinHolder && (
          <LegalOwnerOfRecord
            ...
            formState={currentTransfer.LegalOwnerOfRecordData}
        ...
        {newRegisteredOwnerAddressBlock && (
          <NewRegisteredOwnerAddress
            ...
            selectedOptions={currentTransfer.selectedOptions}
